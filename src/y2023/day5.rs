...
fn parse(input: &str) -> (String, Vec<Vec<Mapping>>) {
    let mut lines = input.lines();
    let seeds = lines.next().unwrap().to_string();
    lines.next();
...
fn part1() {
    ...
    let mut seeds = parse_seeds1(&seeds);
...
fn part2() {
    let (seeds, maps) = parse(&input!());
    let mut seeds = parse_seeds2(&seeds);

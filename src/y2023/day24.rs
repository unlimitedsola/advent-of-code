...
fn part1() {
    ...
    dbg!(solve1(
        &stones,
        &vector![200000000000000f64, 400000000000000f64]
    ));
}
...
fn solve1(stones: &[Hailstone], bound: &Vector2<f64>) -> usize {
    stones
        .iter()
        .tuple_combinations()
...
fn solve2(stones: &[Hailstone]) -> f64 {
    // P + t[i] * V = p[i] + t[i] * v[i]
    // P - p[i] = t[i] * (v[i] - V)
...
fn test_example() {
    ...
    assert_eq!(solve1(&stones, &vector![7.0, 27.0]), 2);
    assert_eq!(solve2(&stones), 47.0);
}

...
fn solve1(sys: &System, parts: &[Part]) -> usize {
    parts
        .iter()
...
#[test]
fn test_example() {
    let (workflows, parts) = parse(EXAMPLE);
    let system = System::new(workflows);
    assert_eq!(solve1(&system, &parts), 19114);
    assert_eq!(solve2(&system), 167409079868000);
}
